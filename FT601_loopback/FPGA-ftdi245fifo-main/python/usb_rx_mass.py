...
if __name__ == '__main__':
    ...
    usb.set_recv_timeout(10000)  # in ms

    total_rx_len = 0
    expect_len = 100 * 1000 * 1000  # randint(1, 10000000) # random a length
    ...
    for i in range(TEST_COUNT):
        data = usb.recv(int(expect_len / TEST_COUNT))  # recv from usb
        rx_len = len(data)
        print(data[0], data[1])
        total_rx_len += rx_len
        ...
        if int(expect_len / TEST_COUNT) != rx_len:
